Newcoin Core version *0.15.1* is now available from:
...
A number of changes to the way Newcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 smalls per kB (0.00001 NCN). This is to prepare for dropping the minimum transaction fee to 0.1 smalls per kB (0.0001 NCN) in the next release.
