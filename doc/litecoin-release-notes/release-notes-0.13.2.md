Newcoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Newcoin transactions now use validation using
...
Previous versions of Newcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Newcoin Core was to raise the default minimum
relay fee.

Newcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Newcoin Core 0.13.2 also introduces new default policy limits on the length and
...
Newcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Newcoin Core will relay transactions with
...
entirely in Newcoin Core 0.13, and it is at this time undecided whether the
...
manual configuration. Newcoin Core will also use Tor automatically to connect
...
Newcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Newcoin
Core to use floating fees. By default, Newcoin Core will use floating
fees.
...
(default: `0.0002` NCN/kB).

At all times, Newcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) NCN.
Furthermore, Newcoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Newcoin Core code base has
...
for a new opcode in the Newcoin scripting system that in combination with
...
Newcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Newcoin Core.
...
The two phases are distinct in the Newcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Newcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Newcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Newcoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Newcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Newcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Newcoin's current ledger (called the UTXO set),
  ...
  note that Newcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Newcoin users to individually opt-in to almost any change in the Newcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Newcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Newcoin
client to Newcoin Core 0.13.2 to signal for segwit support. When a
...
Since Newcoin Core 0.10.0, nodes have defaulted to only relaying and
