Newcoin Core version 0.17.1 is now available from:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 NCN/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
BIP 174 Partially Signed Newcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Newcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcointalk.org/?topic=279249) transactions.

For backend RPC convenience, the Newcoin devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed Newcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Newcoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in NCN. This new field deprecates previous fee fields, such as
