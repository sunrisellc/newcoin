...
- [Development guidelines](#development-guidelines)
    - [General Newcoin Core](#general-newcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Newcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Newcoin Core is a multi-threaded application, and deadlocks or other
...
To profile a running newcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep newcoind` -- sleep 60
```
...
Newcoin Core can be compiled with various "sanitizers" enabled, which add
...
Some of these are maintained by active developers of Newcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Newcoin Core so that they can be integrated
...
file descriptors it will cause problems with Newcoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Newcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Newcoin Core, the wallet was always
