Newcoin Core version 0.18.1 is now available from:

  <https://download.newcoin.org/newcoin-0.18.1/>
...
`/Applications/Newcoin-Qt` (on Mac) or `newcoind`/`newcoin-qt` (on
Linux).
...
Newcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Newcoin Core on unsupported systems.

Newcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Newcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/newcoin-project/newcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@newcoin.org) the Newcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 NCN),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in NCN per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/newcoin-project/newcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/newcoin-project/newcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/newcoin-project/newcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/newcoin-project/newcoin/blob/master/doc/newcoin-conf.md)
  about the `newcoin.conf` file describes how to use it to configure
  Newcoin Core.

- A new document introduces Newcoin Core's BIP174 [Partially-Signed
  Newcoin Transactions
  (PSBT)](https://github.com/newcoin-project/newcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/newcoin-project/newcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Newcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Newcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/newcoin-project/newcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/newcoin-project/newcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Newcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Newcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/newcoin-project/newcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/newcoin-project/newcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart newcoind with the `-deprecatedrpc=generate`
...
- A new `newcoin-wallet` tool is now distributed alongside Newcoin
...
This section describes planned changes to Newcoin Core that may affect
other Newcoin software and services.

- Since version 0.16.0, Newcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Newcoin Core 0.20 (expected about
  a year after 0.18), Newcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Newcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Newcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Newcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Newcoin Core
...
- A sub-project of Newcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Newcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Newcoin Core's own implementation, although entropy
  gathered by Newcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Newcoin Core a
...
- On macOS, Newcoin Core now opts out of application CPU throttling
