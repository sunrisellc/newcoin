# PSBT Howto for Newcoin Core

Since Newcoin Core 0.17, an RPC interface exists for Partially Signed Newcoin
...
PSBT is an interchange format for Newcoin transactions that are not fully signed
...
#### Multisig with multiple Newcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Newcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `newcoin-cli` is used.
